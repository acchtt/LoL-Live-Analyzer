...
function clearStaleCompletionDuringLive(event, broadcastReportedLive, diagnostics) {
  if (!broadcastReportedLive) return;

  const target = seriesTargetWins(event);
  const teams = Array.isArray(event?.match?.teams) ? event.match.teams : [];
  const wins = teams.map(teamWins);
  const scoreIsExplicit = target !== null && wins.length >= 2 && wins.every(value => value !== null);
  const scoreReachedTarget = scoreIsExplicit && wins.some(value => value >= target);
  const hasCompletedFlag = event?.state === 'completed' || event?.match?.state === 'completed';

  // getEventDetails can retain match.state=completed while getLive still reports
  // the same BO3/BO5 and the scoreboard is tied below the winning target. Treat
  // that combination as stale catalog metadata, otherwise the resolver exits
  // before probing the deciding game and the dashboard appears empty.
  if (!scoreIsExplicit || scoreReachedTarget || !hasCompletedFlag) return;

  diagnostics.ignoredStaleCompletion = {
  ...
  if (event.match) event.match.state = 'inProgress';
  if (event.completionSource) delete event.completionSource;
}
...
export async function resolveActiveGame(matchId, riot) {
  ...
  clearStaleCompletionDuringLive(event, broadcastReportedLive, diagnostics);
