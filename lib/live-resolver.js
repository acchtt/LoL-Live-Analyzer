...
export async function resolveActiveGame(matchId, riot) {
  ...
  const initialGameIds = new Set(
    (event?.match?.games || []).map(game => String(game?.id || '')).filter(Boolean)
  );
  ...
  if (liveResult.status === 'fulfilled') {
    ...
    if (liveEvent) {
      for (const game of liveEvent?.match?.games || []) {
        const id = String(game?.id || '');
        if (id && (game?.state === 'inProgress' || !initialGameIds.has(id))) livePriorityIds.add(id);
      }
  ...
  const priorityGames = candidates.filter(game => {
    const id = String(game?.id || '');
    const number = gameNumber(game);
    return number === expected || livePriorityIds.has(id);
  });
