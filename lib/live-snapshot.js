...
export async function buildLiveSnapshot(gameId, env, after, riot) {
  ...
  // Event metadata and the player-details frame are independent once the live
  // window supplies a match ID and timestamp. Loading them together removes a
  // full upstream round trip from every gameplay refresh.
  const eventPromise = matchId
    ? riot.getEvent(matchId).catch(() => null)
    : Promise.resolve(null);
  const detailsPromise = candidate.phase === 'gameplay' && candidate.timestampQuality.freshness !== 'stale'
