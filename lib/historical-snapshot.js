...
const RETAINED_FRAME_SCAN_MINUTES = Array.from({ length: 35 }, (_, index) => (index + 1) * 2);
...
export function retainedFrameScanTimes(anchorTimestamp) {
  const anchorMs = parseTimestamp(anchorTimestamp);
  if (anchorMs === null) return [];
  return uniqueTimes(RETAINED_FRAME_SCAN_MINUTES.map(minute => anchorMs + minute * 60_000))
    .slice(0, MAX_WINDOW_PROBES);
}
...
async function historicalCandidate(gameId, game, riot) {
  ...
  const initial = await inspect(null);
  let cursorTimes = historicalCursorTimes(game);
  if (!cursorTimes.length && initial?.timestampMs) {
    // Some Riot getGames records expose no VOD timing at all. The default retained
    // window often still contains champion select, so use that frame as a bounded
    // anchor and scan forward across the possible game duration.
    cursorTimes = retainedFrameScanTimes(initial.timestampMs);
  }
