...
function normalizePlayer(raw, index, metadataById, detailedById, side, missing) {
  ...
  return {
    ...
    items: (() => {
      if (Array.isArray(combined?.items)) return combined.items;
      missing.push(`${path}.items`);
      return null;
    })()
  };
}
...
export function normalizeGameplay({ candidate, event, gameId, detailedPayload, afterMs }) {
  ...
  const playerCritical = /\.players\.\d+\.(?:level|creepScore|totalGold|items)$/;
  const criticalMissingFields = missingFields.filter(path => criticalExact.has(path) || playerCritical.test(path));
