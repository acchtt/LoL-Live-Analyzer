...
export function createRiotClient(env) {
  ...
  async function fetchBestLiveWindow(gameId, after) {
    ...
    // Always ask Riot's unanchored endpoint first. When it advances, it is the
    // freshest frame Riot is currently publishing even when that regional feed
    // is several minutes behind the broadcast.
    const primary = await attempt(null);
    const primaryAdvances = afterMs === null || (primary && primary.timestampMs > afterMs);
    if (primary?.phase === 'gameplay' && primaryAdvances) return primary;

    // If the unanchored endpoint is pinned to champion select or an old frame,
    // probe both immediately after the displayed frame and against wall-clock
    // delay anchors. Four concurrent probes keep recovery fast without serially
    // waiting through every regional-delay possibility.
    ...
    for (let index = 0; index < probeTimes.length; index += 4) {
      ...
      if (gameplay && advances) return gameplay;
    }
