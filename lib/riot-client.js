...
const INITIAL_LIVE_PROBE_OFFSETS_SECONDS = [20, 60, 120, 240, 300];
const ADVANCING_LIVE_PROBE_OFFSETS_SECONDS = [10, 30, 60];
...
export function liveWindowProbeTimes(nowMs = Date.now(), after = null) {
  ...
  return [...new Set(values
    .filter(value => Number.isFinite(value) && value <= nowMs + FUTURE_TOLERANCE_SECONDS * 1000)
    .map(roundedIso))];
}
...
export function createRiotClient(env) {
  ...
  async function fetchBestLiveWindow(gameId, after) {
    ...
    const primaryAdvances = afterMs === null || (primary && primary.timestampMs > afterMs);
    if (primary?.phase === 'gameplay'
        && primary.timestampQuality.freshness !== 'stale'
        && primaryAdvances) return primary;

    // The no-startingTime endpoint can remain pinned to champion select for
    // some regional feeds. Probe progressively older wall-clock anchors so a
    // feed delayed by a few minutes can still be recovered as context.
    const probeTimes = liveWindowProbeTimes(now, after);
    for (let index = 0; index < probeTimes.length; index += 2) {
      const batch = probeTimes.slice(index, index + 2);
      await Promise.all(batch.map(startingTime => attempt(startingTime)));
      const gameplay = newestCandidate(candidates, 'gameplay');
      const advances = afterMs === null || (gameplay && gameplay.timestampMs > afterMs);
      if (gameplay && gameplay.timestampQuality.freshness !== 'stale' && advances) return gameplay;
    }
