...
const INITIAL_LIVE_PROBE_OFFSETS_SECONDS = [20, 60, 120, 240, 360];
const ADVANCING_LIVE_PROBE_OFFSETS_SECONDS = [10, 20, 30, 60, 120];
const WALL_CLOCK_FALLBACK_OFFSETS_SECONDS = [120, 240, 360];
...
export function createRiotClient(env) {
  ...
  async function fetchBestDetails(gameId, timestampIso) {
    for (const startingTime of detailProbeTimes(timestampIso)) {
      const payload = await fetchDetails(gameId, startingTime).catch(() => null);
      const frame = latestFrame(payload);
      if (frame && Array.isArray(frame?.participants) && frame.participants.length > 0) return payload;
    }
    return null;
  }
  ...
  async function fetchBestLiveWindow(gameId, after) {
    ...
    for (const startingTime of liveWindowProbeTimes(now, after)) {
      const candidate = await attempt(startingTime);
      if (candidate?.phase === 'gameplay' && candidate.timestampQuality.freshness === 'fresh'
          && (afterMs === null || candidate.timestampMs > afterMs)) return candidate;
    }
  ...
  return {
    ...
    fetchDetails,
    getEvent,
    getGames: ids => riotPersisted('getGames', { id: ids }, env),
    getLive: () => riotPersisted('getLive', {}, env),
    getSchedule: leagueId => riotPersisted('getSchedule', { leagueId: leagueId || undefined }, env)
  };
}
